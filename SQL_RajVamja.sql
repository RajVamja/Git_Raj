...
exec SP_procedure6 5, 1
	  ...
	  select * from Orders
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- [7.] Create a Parameterized Store Procedure for Login as per Customer/Salesman EmailId and Password.
...
----------------------------------view for random special charactores---------------------

create or alter view [Random_spe]
as
select char(35 + rand() * 4) as special

select special from [Random_spe]
...
select dbo.FN_GenerateEmail() as [Random Email]
...
begin
	...
	while @j < = 6
		...
		set @special = @special + (select special from [Random_spe])
...
-- foreign key --
alter table customer
add foreign key (id) references customer_1 (id)
...
-- default --
alter table cust123
add constraint df_def default ('raj') for cust_name;
